...
datasets = [
    'FABR/dat/sent_mix/Beauty',
    'FABR/dat/sent_mix/Clothing_Shoes_and_Jewelry',
    'FABR/dat/sent_mix/Movie',
    'FABR/dat/sent_mix/Twitter',
    'FABR/dat/sent_mix/Finance'
            ]
...
def get(logger=None,args=None):
    if os.path.exists('FABR/dat/bin/data_sent_mix'+'_'+str(args.idrandom)+'.pt') and os.path.exists('FABR/dat/bin/taskcla_sent_mix'+'_'+str(args.idrandom)+'.pt'):
        data = torch.load('FABR/dat/bin/data_sent_mix'+'_'+str(args.idrandom)+'.pt',weights_only=False) #setting weights_only=False to override new default behav that raises err opn dgx
        taskcla = torch.load('FABR/dat/bin/taskcla_sent_mix'+'_'+str(args.idrandom)+'.pt',weights_only=False)
        return data,taskcla
    data={}
    taskcla=[]

    # Others
    f_name = 'FABR/asc_random_sent_mix'
    ...
    for t in range(args.ntasks):
        ...
        processor = data_utils.SentMixProcessor()
        ...
        train_features = data_utils.convert_examples_to_features(
            train_examples, label_list, args.max_seq_length, tokenizer, "asc", dataset='sent_mix',convert_to_binary=args.convert_to_binary)
        ...
        valid_features=data_utils.convert_examples_to_features(
            valid_examples, label_list, args.max_seq_length, tokenizer, "asc", dataset='sent_mix',convert_to_binary=args.convert_to_binary)
        ...
        eval_features = data_utils.convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer, "asc", dataset='sent_mix',convert_to_binary=args.convert_to_binary)
    ...
    torch.save(data,'FABR/dat/bin/data_sent_mix'+'_'+str(args.idrandom)+'.pt')
    torch.save(taskcla,'FABR/dat/bin/taskcla_sent_mix'+'_'+str(args.idrandom)+'.pt')
    return data,taskcla
